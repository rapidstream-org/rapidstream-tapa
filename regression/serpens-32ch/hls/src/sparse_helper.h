...
void edge_list_64bit(
    ...
    for (int cc = 0; cc < NUM_CH_SPARSE; ++cc) {
        ...
        sparse_A_fpga_vec[cc] = vector<unsigned long, aligned_allocator<unsigned long>> (sparse_A_fpga_chunk_size, x_const);
    }
    
    // col(12 bits) + row (20 bits) + value (32 bits)
    // ->
    // col(14 bits) + row (18 bits) + value (32 bits)
    //cout << "##########################\nHost generated edge list\n#########################\n";
    for (int cc = 0; cc < NUM_CH_SPARSE; ++cc) {
        for (int j = 0; j < 8; ++j) {
            ...
            for (int i = 0; i < edge_list_ptr[edge_list_ptr.size()-1]; ++i) {
                ...
                if (e.row != -1) {
                    ...
                    sparse_A_fpga_vec[ch_idx][pe_idx / 32 + i * 8] = x_col | x_row | x_float_val_64;
                }
            }
        }
    }
