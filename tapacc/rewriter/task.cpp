...
static std::map<TapaTargetAttr::TargetType,
    ...
    target_map{
        ...
        {TapaTargetAttr::TargetType::NON_SYNTHETIC,
         {
             {TapaTargetAttr::VendorType::Xilinx,
              XilinxNonSyntheticTarget::GetInstance()},
         }},
    };
...
bool Visitor::VisitFunctionDecl(FunctionDecl* func) {
  ...
  if (func->isGlobal() &&
    ...
    if (is_first_traversal) {
      ...
      // if the task is non-synthetic, it is a lower-level tapa task.
      if (IsTaskNonSynthetic(func)) {
        is_upper_level_task = false;
        if (is_top_level_task) {
          static const auto diagnostic_id =
              this->context_.getDiagnostics().getCustomDiagID(
                  clang::DiagnosticsEngine::Error,
                  "tapa top-level task cannot be non-synthetic");
