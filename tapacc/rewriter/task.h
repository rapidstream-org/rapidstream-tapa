...
// Check if a task has non-synthetic label.
inline bool IsTaskNonSynthetic(const clang::FunctionDecl* func) {
  if (auto attr = func->getAttr<clang::TapaTargetAttr>()) {
    if (attr->getTarget() == clang::TapaTargetAttr::TargetType::NON_SYNTHETIC) {
      return true;
    }
  }
  return false;
...
inline std::vector<const clang::FunctionDecl*> FindChildrenTasks(
  ...
  // when a function is non-synthetic, it does not have any children.
  if (IsTaskNonSynthetic(upper)) {
    return {};
  }
