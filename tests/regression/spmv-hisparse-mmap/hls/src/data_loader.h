...
template <typename data_type>
CSRMatrix<data_type> create_csr_matrix(
...
// Load a csr matrix from a scipy sparse npz file. The sparse matrix should have
// float data type.
CSRMatrix<float> load_csr_matrix_from_float_npz(
    std::string csr_float_npz_path) {
  ...
  csr_matrix.adj_data.insert(csr_matrix.adj_data.begin(),
                             &npy_data.data<float>()[0],
                             &npy_data.data<float>()[nnz]);
  csr_matrix.adj_indices.insert(csr_matrix.adj_indices.begin(),
                                &npy_indices.data<uint32_t>()[0],
                                &npy_indices.data<uint32_t>()[nnz]);
  csr_matrix.adj_indptr.insert(csr_matrix.adj_indptr.begin(),
                               &npy_indptr.data<uint32_t>()[0],
...
CSRMatrix<data_type> csr_matrix_convert_from_float(CSRMatrix<float> const& in) {
  ...
  std::copy(in.adj_data.begin(), in.adj_data.end(),
            std::back_inserter(out.adj_data));
...
CSCMatrix<data_type> csr2csc(CSRMatrix<data_type> const& csr_matrix) {
  ...
  for (size_t col_idx = 0; col_idx < csc_matrix.num_cols; col_idx++) {
    csc_matrix.adj_indptr[col_idx + 1] =
        csc_matrix.adj_indptr[col_idx] + nnz_each_col[col_idx];
  }
  ...
  for (size_t row_idx = 0; row_idx < csr_matrix.num_rows; row_idx++) {
    for (size_t i = csr_matrix.adj_indptr[row_idx];
         i < csr_matrix.adj_indptr[row_idx + 1]; i++) {
      uint32_t col_idx = csr_matrix.adj_indices[i];
      uint32_t dest =
          csc_matrix.adj_indptr[col_idx] + nnz_consumed_each_col[col_idx];
