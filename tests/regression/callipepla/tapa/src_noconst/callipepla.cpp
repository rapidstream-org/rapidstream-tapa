...
void PEG_Yvec(tapa::istream<int>& fifo_inst_in,
  ...
  const int num_v_init =
      (local_M + NUM_CH_SPARSE_mult_8 - 1) / NUM_CH_SPARSE_mult_8;
...
// M end: res = r' * r
void dot_res(tapa::istream<double_v8>& q1, tapa::ostream<ResTerm>& q2,
...
// M6: rznew = r' * z
void dot_rznew(tapa::istream<double_v8>& qr, tapa::istream<double_v8>& qz,
               tapa::ostream<double_v8>& qr_out, tapa::ostreams<double, 2>& qrz,
               tapa::istream<bool>& q_gbc, tapa::ostream<bool>& q_gbc_out) {
  // InstCmp inst = {(M + 7) >> 3, 0.0, 0};
  const int num_ite = (M + 7) >> 3;
  bool term_flag = false;
...
// M3: x = x + alpha * p
void updt_x(tapa::istream<double>& qalpha, tapa::istream<double_v8>& qx,
            tapa::istream<double_v8>& qp, tapa::ostream<double_v8>& qout,
...
// M4: r = r - alpha * Ap
void updt_r(tapa::istream<double>& qalpha, tapa::istream<double_v8>& qr,
            tapa::istream<double_v8>& qap, tapa::ostream<double_v8>& qout,
            tapa::istream<bool>& q_gbc, tapa::ostream<bool>& q_gbc_out) {
...
// M5: z = diagA \ r
void left_div(tapa::istream<double_v8>& qr, tapa::istream<double_v8>& qdiagA,
...
void Callipepla(tapa::mmap<int> edge_list_ptr,
  ...
  tapa::task()
      ...
      .invoke<tapa::join>(read_edge_list_ptr, edge_list_ptr, PE_inst,
                          tsignal_edgepointer, tsignal_edgepointer)

      // P
      .invoke<tapa::detach, 2>(rdwr_vec, vec_p, fifo_mi_P, fifo_dout_P,
                               fifo_din_P, fifo_resp_P)

      .invoke<tapa::join>(ctrl_P, fifo_din_P, fifo_dout_P, fifo_mi_P,
                          fifo_P_from_mem, fifo_P_dot,
                          // fifo_P_updtx,
                          fifo_P_updtp, fifo_P_updated, fifo_resp_P,
                          tsignal_ctrlP, tsignal_ctrlAP)
      // p mux
      .invoke<tapa::join>(vecp_mux, tsignal_mux, fifo_P_from_mem, fifo_P_to_mux,
                          fifo_P_pe)

      .invoke<tapa::join, NUM_CH_SPARSE>(read_A, edge_list_ch, fifo_A,
                                         tsignal_rdA, tsignal_rdA)
      ...
      .invoke<tapa::join, 8>(Arbiter_Y, fifo_Y_pe, fifo_Y_pe_abd, tsignal_aby,
                             tsignal_aby)
      ...
      .invoke<tapa::join>(ctrl_R, fifo_din_R, fifo_dout_R, fifo_mi_R, fifo_R,
                          fifo_R_tomem, fifo_resp_R, tsignal_ctrlR,
                          tsignal_ctrlX)

      // digA
      .invoke<tapa::join>(read_digA, vec_digA, fifo_dA, tsignal_ctrldigA,
                          tsignal_ctrlR)
      ...
      .invoke<tapa::join>(ctrl_X, fifo_din_X, fifo_dout_X, fifo_mi_X, fifo_X,
                          fifo_X_updated, fifo_resp_X, tsignal_ctrlX)

      // AP
      .invoke<tapa::detach>(rdwr_vec, vec_Ap, fifo_mi_AP, fifo_dout_AP,
                            fifo_din_AP, fifo_resp_AP)

      .invoke<tapa::join>(ctrl_AP, fifo_din_AP, fifo_dout_AP, fifo_mi_AP,
                          fifo_AP, fifo_AP_M1, fifo_resp_AP, tsignal_ctrlAP,
                          tsignal_ctrldigA)
      ...
      // M3: x = x + alpha * p
      .invoke<tapa::join>(updt_x, fifo_alpha, fifo_X, fifo_P_updtx,
                          fifo_X_updated, tsignal_toM3)

      // M4: r = r - alpha * Ap
      .invoke<tapa::join>(updt_r, fifo_alpha, fifo_R, fifo_AP, fifo_R_updtd_m5,
                          tsignal_toM4, tsignal_toM5)

      // M5: z = diagA \ r
      .invoke<tapa::join>(left_div, fifo_R_updtd_m5, fifo_dA, fifo_Z,
                          fifo_R_updtd_m6, fifo_R_tomem, tsignal_toM5,
                          tsignal_toM6)

      // M6: rznew = r' * z
      .invoke<tapa::join>(dot_rznew, fifo_R_updtd_m6, fifo_Z, fifo_R_updtd_rr,
                          fifo_rz, tsignal_toM6, tsignal_toM7)
      ...
      // M residual
      .invoke<tapa::join>(dot_res, fifo_R_updtd_rr, fifo_RR, tsignal_res)

      .invoke<tapa::join>(wr_r, vec_res, fifo_RR);
}
