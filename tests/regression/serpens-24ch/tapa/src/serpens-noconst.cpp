...
#pragma HLS bind_storage variable = local_X type = RAM_2P impl = \
    BRAM latency = 2
...
#pragma HLS loop_tripcount min = 1 max = 49
    ...
    read_X:
      for (int j = 0; (j < WINDOW_SIZE_div_16) &
                      (j < ((local_K + 15) >> 4) - i * WINDOW_SIZE_div_16);) {
...
void PEG_Yvec(tapa::istream<int>& fifo_inst_in,
  ...
  const int num_v_init =
      (local_M + NUM_CH_SPARSE_mult_16 - 1) / NUM_CH_SPARSE_mult_16;
  const int num_v_out =
      (local_M + NUM_CH_SPARSE_mult_2 - 1) / NUM_CH_SPARSE_mult_2;
...
void Serpens(tapa::mmap<int> edge_list_ptr,
  ...
  tapa::task()
      .invoke(read_edge_list_ptr, edge_list_ptr, PE_inst)

      .invoke<tapa::join>(read_X, vec_X, fifo_X_pe)
      ...
      .invoke<tapa::join, 8>(Arbiter_Y, fifo_Y_pe, fifo_Y_pe_abd)

      .invoke<tapa::detach>(Merger_Y, fifo_Y_pe_abd, fifo_Y_AX)

      .invoke<tapa::join>(FloatvMultConst_alpha, fifo_Y_AX, fifo_Y_alpha_AX)

      .invoke<tapa::join>(read_Y, vec_Y, fifo_Y_in)

      .invoke<tapa::join>(FloatvMultConst_beta, fifo_Y_in, fifo_Y_in_beta)

      .invoke<tapa::detach>(FloatvAddFloatv, fifo_Y_alpha_AX, fifo_Y_in_beta,
                            fifo_Y_out)

      .invoke<tapa::join>(write_Y, fifo_Y_out, vec_Y_out);
}
