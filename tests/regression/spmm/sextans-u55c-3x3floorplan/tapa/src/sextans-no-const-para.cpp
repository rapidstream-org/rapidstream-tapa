...
#pragma HLS bind_storage variable = local_B type = RAM_2P impl = \
    BRAM latency = 2
...
#pragma HLS loop_tripcount min=1 max=49
            ...
            // fill onchip B
        read_B:
            for (int j = 0; (j < (WINDOW_SIZE >> 3)) && (j < ((local_K + 7) >>
3) - i * (WINDOW_SIZE >> 3)); ) { #pragma HLS loop_tripcount min=1 max=512
#pragma HLS pipeline style=stp II = 1
