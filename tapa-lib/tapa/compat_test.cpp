...
void DataSink(tapa::hls::stream<int>& data_in_q, int n) {
  for (int i = 0; i < n; ++i) {
    data_in_q.read();
  }
}

void DataSource(tapa::hls::stream<int>& data_out_q, int n) {
  for (int i = 0; i < n; ++i) {
    data_out_q.write(i);
  }
}
...
void DataSourceNonBlocking(tapa::ostream<float>& data_out_q,
                           tapa::hls::stream<int>& count_out_q, int n) {
  int write_count = 0;
  for (int i = 0; i < n; ++i) {
...
void DataSinkNonBlocking(tapa::istream<float>& data_in_q,
                         tapa::hls::stream<int>& count_in_q, int n) {
  int read_count = 0;
  for (int i = 0; i < n; ++i) {
    float value_unused;
...
TEST(SimulateTest, SimulateHlsTasksAreScheduledSequentially) {
  ...
  tapa::hls::stream<int> count_q;
  DataSourceNonBlocking(data_q, count_q, kN);
  DataSinkNonBlocking(data_q, count_q, kN);
}
