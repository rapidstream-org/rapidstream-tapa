...
def get_top_level_ab_graph(program: Program) -> ABGraph:
    ...
    return add_port_connections(program, graph, port_width)
...
def collect_port_width(program: Program) -> dict[str, int]:
    ...
    for port in program.top_task.ports.values():
        if not any((port.is_streams, port.is_immap, port.is_ommap)):
            continue
...
def collect_task_area(program: Program) -> dict[str, dict[str, int]]:
    ...
    for task_name in program.top_task.tasks:
        areas[task_name] = program.get_area(task_name)
...
def add_port_connections(
    program: Program, graph: ABGraph, port_width: dict[str, int]
) -> ABGraph:
    """Add port connections to the ab graph.
