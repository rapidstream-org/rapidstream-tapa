        ...
        for field in pblock_fields:
            if field is not None:
                for pblock_line in field:
                    ...
                    msg = f"Invalid pblock: {pblock_line}, must start with -add or -remove"
                    raise ValueError(msg)
        ...
        for x in range(self.cols):
            for y in range(self.rows):
                ...
                if slot.area.is_empty():
                    _logger.warning(f"{name} has an empty area.")

                if slot.pblock_ranges is None:
                    _logger.warning(f"{name} does not have a pblock range.")

                if y < self.rows - 1:
                    if slot.north_wire_capacity == WIRE_CAPACITY_INF:
                        _logger.warning(f"{name} has infinite north wires.")
                    if not slot.north_anchor_region:
                        _logger.warning(f"{name} does not have north anchor region.")

                if y > 0:
                    if slot.south_wire_capacity == WIRE_CAPACITY_INF:
                        _logger.warning(f"{name} has infinite south wire capacity.")
                    if not slot.south_anchor_region:
                        _logger.warning(f"{name} does not have south anchor region.")

                if x < self.cols - 1:
                    if slot.east_wire_capacity == WIRE_CAPACITY_INF:
                        _logger.warning(f"{name} has infinite east wire capacity.")
                    if not slot.east_anchor_region:
                        _logger.warning(f"{name} does not have east anchor region.")

                if x > 0:
                    if slot.west_wire_capacity == WIRE_CAPACITY_INF:
                        _logger.warning(f"{name} has infinite west wire capacity.")
                    if not slot.west_anchor_region:
                        _logger.warning(f"{name} does not have west anchor region.")
