...
def get_graphir_iface(  # noqa: PLR0914, C901
    ...
    for module in modules:
        ...
        ifaces[module.name] = list(set(updated_ifaces))
...
def set_iface_role(module: AnyModuleDefinition, iface: AnyInterface) -> AnyInterface:
    ...
    return set_iface_role_by_ports(name_to_port, iface, module.name)


def set_hs_iface_role_by_ports(
...
def set_ap_ctrl_iface_role_by_ports(
...
def set_iface_role_by_ports(
    ...
    if isinstance(iface, (HandShakeInterface)):
        return set_hs_iface_role_by_ports(name_to_port, iface, module_name)

    # for ap_ctrl interfaces, the role is determined by ap_start
    if isinstance(iface, (ApCtrlInterface)):
        return set_ap_ctrl_iface_role_by_ports(name_to_port, iface, module_name)
