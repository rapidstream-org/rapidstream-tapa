...
def get_verilog_module_from_leaf_task(task: Task) -> VerilogModuleDefinition:
    ...
    return VerilogModuleDefinition(
        ...
        parameters=tuple(get_task_graphir_parameters(task.module)),
        ports=tuple(get_task_graphir_ports(task.module)),
...
def get_submodule_inst(
    ...
    for signal in ap_signals:
        ModuleConnection(
            ...
            expr=Expression((Token.new_id(f"{task_name}_{signal}"),)),
        )
    ...
    return ModuleInstantiation(
        ...
        connections=(
            ...
            ModuleConnection(
                ...
                expr=Expression((Token.new_id(f"{fifo_name}__dout"),)),
            ),
            ModuleConnection(
                name="if_empty_n",
                hierarchical_name=HierarchicalName.get_name("if_empty_n"),
                expr=Expression((Token.new_id(f"{fifo_name}__empty_n"),)),
            ),
            ModuleConnection(
                name="if_read",
                hierarchical_name=HierarchicalName.get_name("if_read"),
                expr=Expression((Token.new_id(f"{fifo_name}__read"),)),
            ),
            ModuleConnection(
                name="if_read_ce",
            ...
            ModuleConnection(
                ...
                expr=Expression((Token.new_id(f"{fifo_name}__din"),)),
            ),
            ModuleConnection(
                name="if_full_n",
                hierarchical_name=HierarchicalName.get_name("if_full_n"),
                expr=Expression((Token.new_id(f"{fifo_name}__full_n"),)),
            ),
            ModuleConnection(
                name="if_write",
                hierarchical_name=HierarchicalName.get_name("if_write"),
                expr=Expression((Token.new_id(f"{fifo_name}__write"),)),
            ),
            ModuleConnection(
                name="if_write_ce",
...
def get_upper_task_ir_wires(
    ...
    port_range_mapping = {port.name: port.range for port in upper_task_ir_ports}
...
def get_top_module_definition(
    ...
    top_subinsts = get_upper_module_ir_subinsts(
        top,
        slot_defs,
    )
