...
static unsigned getFunctionOrMethodNumParams(const Decl *D) {
  ...
  if (const auto *BD = dyn_cast<BlockDecl>(D)) return BD->getNumParams();
  return cast<ObjCMethodDecl>(D)->param_size();
}

static const ParmVarDecl *getFunctionOrMethodParam(const Decl *D,
                                                   unsigned Idx) {
  if (const auto *FD = dyn_cast<FunctionDecl>(D)) return FD->getParamDecl(Idx);
  if (const auto *MD = dyn_cast<ObjCMethodDecl>(D))
    return MD->getParamDecl(Idx);
  if (const auto *BD = dyn_cast<BlockDecl>(D)) return BD->getParamDecl(Idx);
  return nullptr;
}
...
static bool isFunctionOrMethodVariadic(const Decl *D) {
  ...
  if (const auto *BD = dyn_cast<BlockDecl>(D)) return BD->isVariadic();
...
static inline bool isCFStringType(QualType T, ASTContext &Ctx) {
  ...
  if (RD->getTagKind() != TTK_Struct) return false;
...
static bool checkPositiveIntArgument(Sema &S, const AttrInfo &AI,
  ...
  uint32_t UVal;
  if (!checkUInt32Argument(S, AI, Expr, UVal, Idx)) return false;
...
bool Sema::checkStringLiteralArgumentAttr(const ParsedAttr &AL, unsigned ArgNum,
  ...
  if (AL.isArgIdent(ArgNum)) {
    ...
    if (ArgLocation) *ArgLocation = Loc->Loc;
    return true;
  }
  ...
  if (ArgLocation) *ArgLocation = ArgExpr->getBeginLoc();
...
static void handleTapaTargetAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context)
                 TapaTargetAttr(AL.getRange(), S.Context, Target, Vendor,
                                AL.getAttributeSpellingListIndex()));
}

template <typename... DiagnosticArgs>
static const Sema::SemaDiagnosticBuilder &appendDiagnostics(
...
static const Sema::SemaDiagnosticBuilder &appendDiagnostics(
    const Sema::SemaDiagnosticBuilder &Bldr, T &&ExtraArg,
...
static void handleSimpleAttributeOrDiagnose(Sema &S, Decl *D, SourceRange SR,
                                            unsigned SpellingIndex,
...
static void handleSimpleAttributeWithExclusions(Sema &S, Decl *D,
                                                const ParsedAttr &AL) {
  if (checkAttrMutualExclusion<IncompatibleAttrType>(S, D, AL)) return;
...
static bool threadSafetyCheckIsSmartPointer(Sema &S, const RecordType *RT) {
  ...
  if (foundStarOperator && foundArrowOperator) return true;
...
static bool threadSafetyCheckIsPointer(Sema &S, const Decl *D,
  ...
  if (QT->isAnyPointerType()) return true;

  if (const auto *RT = QT->getAs<RecordType>()) {
    // If it's an incomplete type, it could be a smart pointer; skip it.
    // (We don't want to force template instantiation if we can avoid it,
    // since that would alter the order in which templates are instantiated.)
    if (RT->isIncompleteType()) return true;

    if (threadSafetyCheckIsSmartPointer(S, RT)) return true;
  }
...
static const RecordType *getRecordType(QualType QT) {
  if (const auto *RT = QT->getAs<RecordType>()) return RT;
...
static bool checkRecordDeclForAttr(const RecordDecl *RD) {
  ...
  if (RD->hasAttr<AttrType>()) return true;
...
static bool typeHasCapability(Sema &S, QualType Ty) {
  if (checkTypedefTypeForCapability(Ty)) return true;

  if (checkRecordTypeForCapability(S, Ty)) return true;
...
static void handlePtGuardedVarAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (!threadSafetyCheckIsPointer(S, D, AL)) return;

  D->addAttr(::new (S.Context) PtGuardedVarAttr(
...
static void handleGuardedByAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  Expr *Arg = nullptr;
  if (!checkGuardedByAttrCommon(S, D, AL, Arg)) return;
...
static bool checkAcquireOrderAttrCommon(Sema &S, Decl *D, const ParsedAttr &AL,
                                        SmallVectorImpl<Expr *> &Args) {
  if (!checkAttributeAtLeastNumArgs(S, AL, 1)) return false;
...
static void handleAcquiredAfterAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  SmallVector<Expr *, 1> Args;
  if (!checkAcquireOrderAttrCommon(S, D, AL, Args)) return;
...
static void handleAssertSharedLockAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  SmallVector<Expr *, 1> Args;
  if (!checkLockFunAttrCommon(S, D, AL, Args)) return;
...
static void handleAllocSizeAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkPositiveIntArgument(S, AL, SizeExpr, SizeArgNoVal, /*Index=*/1))
    return;
  if (!checkParamIsIntegerType(S, FD, AL, /*AttrArgNo=*/0)) return;
  ...
  if (AL.getNumArgs() == 2) {
    ...
    if (!checkPositiveIntArgument(S, AL, NumberExpr, Val, /*Index=*/2)) return;
    if (!checkParamIsIntegerType(S, FD, AL, /*AttrArgNo=*/1)) return;
...
static void handleSharedTrylockFunctionAttr(Sema &S, Decl *D,
  ...
  if (!checkTryLockFunAttrCommon(S, D, AL, Args)) return;
...
static void handleLockReturnedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) LockReturnedAttr(
      AL.getRange(), S.Context, Args[0], AL.getAttributeSpellingListIndex()));
}

static void handleLocksExcludedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (!checkAttributeAtLeastNumArgs(S, AL, 1)) return;
...
static bool checkFunctionConditionAttr(Sema &S, Decl *D, const ParsedAttr &AL,
  ...
  if (!Cond->isTypeDependent()) {
    ...
    if (Converted.isInvalid()) return false;
    Cond = Converted.get();
  }

  if (!S.checkStringLiteralArgumentAttr(AL, 1, Msg)) return false;

  if (Msg.empty()) Msg = "<no message provided>";
...
static void handleDiagnoseIfAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  Expr *Cond;
  StringRef Msg;
  if (!checkFunctionConditionAttr(S, D, AL, Cond, Msg)) return;

  StringRef DiagTypeStr;
  if (!S.checkStringLiteralArgumentAttr(AL, 2, DiagTypeStr)) return;
...
static void handlePassObjectSizeAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  uint32_t Type;
  if (!checkUInt32Argument(S, AL, E, Type, /*Idx=*/1)) return;
...
static void handleCallableWhenAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkForConsumableClass(S, cast<CXXMethodDecl>(D), AL)) return;
  ...
  D->addAttr(::new (S.Context) CallableWhenAttr(
      AL.getRange(), S.Context, States.data(), States.size(),
      AL.getAttributeSpellingListIndex()));
}
...
static void handleSetTypestateAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) SetTypestateAttr(
      AL.getRange(), S.Context, NewState, AL.getAttributeSpellingListIndex()));
}
...
static void handleTestTypestateAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) TestTypestateAttr(
      AL.getRange(), S.Context, TestState, AL.getAttributeSpellingListIndex()));
}
...
static void handleIBOutlet(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (!checkIBOutletCommon(S, D, AL)) return;
...
static void handleIBOutletCollection(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!QTLoc) QTLoc = S.Context.getTrivialTypeSourceInfo(QT, AL.getLoc());
  ...
  D->addAttr(::new (S.Context) IBOutletCollectionAttr(
      AL.getRange(), S.Context, QTLoc, AL.getAttributeSpellingListIndex()));
}
  ...
  if (const RecordType *UT = T->getAsUnionType()) {
    if (UT && UT->getDecl()->hasAttr<TransparentUnionAttr>()) {
      ...
      for (const auto *I : UD->fields()) {
        ...
        if (QT->isAnyPointerType() || QT->isBlockPointerType()) return true;
      }
    }
  }
...
static void handleNonNullAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned I = 0; I < AL.getNumArgs(); ++I) {
    ...
    ParamIdx Idx;
    if (!checkFunctionOrMethodParameterIndex(S, D, AL, I + 1, Ex, Idx)) return;
...
static void handleNonNullAttrParameter(Sema &S, ParmVarDecl *D,
  ...
  D->addAttr(::new (S.Context) NonNullAttr(AL.getRange(), S.Context, nullptr, 0,
                                           AL.getAttributeSpellingListIndex()));
}
...
static void handleReturnsNonNullAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) ReturnsNonNullAttr(
      AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}
...
void Sema::AddAssumeAlignedAttr(SourceRange AttrRange, Decl *D, Expr *E,
  ...
  D->addAttr(::new (Context) AssumeAlignedAttr(AttrRange, Context, E, OE,
                                               SpellingListIndex));
}
...
static void handleOwnershipAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned i = 1; i < AL.getNumArgs(); ++i) {
    Expr *Ex = AL.getArgAsExpr(i);
    ParamIdx Idx;
    if (!checkFunctionOrMethodParameterIndex(S, D, AL, i, Ex, Idx)) return;
    ...
    switch (K) {
      case OwnershipAttr::Takes:
      case OwnershipAttr::Holds:
        if (!T->isAnyPointerType() && !T->isBlockPointerType()) Err = 0;
        break;
      case OwnershipAttr::Returns:
        if (!T->isIntegerType()) Err = 1;
        break;
    }
    if (-1 != Err) {
...
static void handleWeakRefAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) WeakRefAttr(AL.getRange(), S.Context,
                                           AL.getAttributeSpellingListIndex()));
}

static void handleIFuncAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  StringRef Str;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, Str)) return;
...
static void handleAliasAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!S.LangOpts.CPlusPlus) {
    ...
    if (S.LookupQualifiedName(LR, S.getCurLexicalContext()))
      for (NamedDecl *ND : LR) ND->markUsed(S.Context);
  }
...
static void handleTLSModelAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!S.checkStringLiteralArgumentAttr(AL, 0, Model, &LiteralLoc)) return;
  ...
  D->addAttr(::new (S.Context) TLSModelAttr(
...
static void handleCommonAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (CommonAttr *CA = S.mergeCommonAttr(D, AL)) D->addAttr(CA);
}

static void handleNakedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (checkAttrMutualExclusion<DisableTailCallsAttr>(S, D, AL)) return;
...
static void handleUnusedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) UnusedAttr(AL.getRange(), S.Context,
                                          AL.getAttributeSpellingListIndex()));
}
...
static void handleConstructorAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) ConstructorAttr(
      AL.getRange(), S.Context, priority, AL.getAttributeSpellingListIndex()));
}
...
static void handleDestructorAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) DestructorAttr(
      AL.getRange(), S.Context, priority, AL.getAttributeSpellingListIndex()));
}
...
static void handleObjCSuppresProtocolAttr(Sema &S, Decl *D,
  ...
  D->addAttr(::new (S.Context) ObjCExplicitProtocolImplAttr(
      AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}
...
static bool checkAvailabilityAttr(Sema &S, SourceRange Range,
  ...
  if (PlatformName.empty()) PlatformName = Platform->getName();
...
static bool versionsMatch(const VersionTuple &X, const VersionTuple &Y,
  ...
  if (BeforeIsOkay && X < Y) return true;
...
AvailabilityAttr *Sema::mergeAvailabilityAttr(
    ...
    AvailabilityMergeKind AMK, unsigned AttrSpellingListIndex) {
  ...
  if (D->hasAttrs()) {
    ...
    for (unsigned i = 0, e = Attrs.size(); i != e;) {
      ...
      if (MergedIntroduced2.empty()) MergedIntroduced2 = OldIntroduced;
      if (MergedDeprecated2.empty()) MergedDeprecated2 = OldDeprecated;
      if (MergedObsoleted2.empty()) MergedObsoleted2 = OldObsoleted;
  ...
  if (!checkAvailabilityAttr(*this, Range, Platform, MergedIntroduced,
    ...
    auto *Avail = ::new (Context) AvailabilityAttr(
        Range, Context, Platform, Introduced, Deprecated, Obsoleted,
        IsUnavailable, Message, IsStrict, Replacement, AttrSpellingListIndex);
...
static void handleAvailabilityAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (!checkAttributeNumArgs(S, AL, 1)) return;
  ...
  AvailabilityAttr *NewAttr = S.mergeAvailabilityAttr(
      ...
      Replacement, Sema::AMK_None, Index);
  if (NewAttr) D->addAttr(NewAttr);
...
static T *mergeVisibilityAttr(Sema &S, Decl *D, SourceRange range,
  ...
  if (existingAttr) {
    ...
    if (existingValue == value) return nullptr;
...
static void handleVisibilityAttr(Sema &S, Decl *D, const ParsedAttr &AL,
  ...
  StringRef TypeStr;
  SourceLocation LiteralLoc;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, TypeStr, &LiteralLoc)) return;
...
static void handleObjCIndependentClass(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) ObjCIndependentClassAttr(
      AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}
...
static void handleBlocksAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) BlocksAttr(AL.getRange(), S.Context, type,
                                          AL.getAttributeSpellingListIndex()));
}
...
static void handleWorkGroupSize(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context)
                 WorkGroupAttr(AL.getRange(), S.Context, WGSize[0], WGSize[1],
                               WGSize[2], AL.getAttributeSpellingListIndex()));
}
...
static void handleSubGroupSize(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkUInt32Argument(S, AL, E, SGSize)) return;
  if (SGSize == 0) {
    S.Diag(AL.getLoc(), diag::err_attribute_argument_is_zero)
        << AL << E->getSourceRange();
    return;
  ...
  D->addAttr(::new (S.Context) OpenCLIntelReqdSubGroupSizeAttr(
      AL.getRange(), S.Context, SGSize, AL.getAttributeSpellingListIndex()));
}
...
static void handleVecTypeHint(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!ParmType->isExtVectorType() && !ParmType->isFloatingType() &&
    ...
    S.Diag(AL.getLoc(), diag::err_attribute_argument_vec_type_hint) << ParmType;
    return;
  }
  ...
  D->addAttr(::new (S.Context) VecTypeHintAttr(
      AL.getLoc(), S.Context, ParmTSI, AL.getAttributeSpellingListIndex()));
}

SectionAttr *Sema::mergeSectionAttr(Decl *D, SourceRange Range, StringRef Name,
                                    unsigned AttrSpellingListIndex) {
  ...
  if (SectionAttr *ExistingAttr = D->getAttr<SectionAttr>()) {
    if (ExistingAttr->getName() == Name) return nullptr;
  ...
  return ::new (Context)
      SectionAttr(Range, Context, Name, AttrSpellingListIndex);
}
...
static void handleSectionAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  StringRef Str;
  SourceLocation LiteralLoc;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, Str, &LiteralLoc)) return;

  if (!S.checkSectionName(LiteralLoc, Str)) return;
...
CodeSegAttr *Sema::mergeCodeSegAttr(Decl *D, SourceRange Range, StringRef Name,
                                    unsigned AttrSpellingListIndex) {
  // Explicit or partial specializations do not inherit
  // the code_seg attribute from the primary template.
  if (const auto *FD = dyn_cast<FunctionDecl>(D)) {
    if (FD->isFunctionTemplateSpecialization()) return nullptr;
  }
  ...
  return ::new (Context)
      CodeSegAttr(Range, Context, Name, AttrSpellingListIndex);
}
...
static void handleCodeSegAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  SourceLocation LiteralLoc;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, Str, &LiteralLoc)) return;
  if (!checkCodeSegName(S, LiteralLoc, Str)) return;
  if (const auto *ExistingAttr = D->getAttr<CodeSegAttr>()) {
...
static void handleMinVectorWidthAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) MinVectorWidthAttr(
      AL.getRange(), S.Context, VecWidth, AL.getAttributeSpellingListIndex()));
}
...
static void handleCleanupAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (auto *DRE = dyn_cast<DeclRefExpr>(E)) {
    if (DRE->hasQualifier()) S.Diag(Loc, diag::warn_cleanup_ext);
    ...
    if (ULE->hasExplicitTemplateArgs()) S.Diag(Loc, diag::warn_cleanup_ext);
  ...
  D->addAttr(::new (S.Context) CleanupAttr(AL.getRange(), S.Context, FD,
                                           AL.getAttributeSpellingListIndex()));
}
...
static void handleFormatArgAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  ParamIdx Idx;
  if (!checkFunctionOrMethodParameterIndex(S, D, AL, 1, IdxExpr, Idx)) return;
...
static void handleInitPriorityAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (S.Context.getAsArrayType(T)) T = S.Context.getBaseElementType(T);
...
FormatAttr *Sema::mergeFormatAttr(Decl *D, SourceRange Range,
  ...
  for (auto *F : D->specific_attrs<FormatAttr>()) {
    if (F->getType() == Format && F->getFormatIdx() == FormatIdx &&
      ...
      // location.
      if (F->getLocation().isInvalid()) F->setRange(Range);
      return nullptr;
    }
  }
...
static void handleFormatAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  uint32_t Idx;
  if (!checkUInt32Argument(S, AL, IdxExpr, Idx, 2)) return;
  ...
  uint32_t FirstArg;
  if (!checkUInt32Argument(S, AL, FirstArgExpr, FirstArg, 3)) return;
  ...
  FormatAttr *NewAttr = S.mergeFormatAttr(D, AL.getRange(), II, Idx, FirstArg,
                                          AL.getAttributeSpellingListIndex());
  if (NewAttr) D->addAttr(NewAttr);
}
...
static void handleTransparentUnionAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (FirstType->isIncompleteType()) return;
  uint64_t FirstSize = S.Context.getTypeSize(FirstType);
  uint64_t FirstAlign = S.Context.getTypeAlign(FirstType);
  for (; Field != FieldEnd; ++Field) {
    QualType FieldType = Field->getType();
    if (FieldType->isIncompleteType()) return;
...
static void handleAnnotateAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (const auto *I : D->specific_attrs<AnnotateAttr>()) {
    if (I->getAnnotation() == Str) return;
  }

  D->addAttr(::new (S.Context) AnnotateAttr(
...
void Sema::AddAlignValueAttr(SourceRange AttrRange, Decl *D, Expr *E,
  ...
  if (!E->isValueDependent()) {
    ...
    D->addAttr(::new (Context) AlignValueAttr(AttrRange, Context, ICE.get(),
                                              SpellingListIndex));
    return;
  }
...
static void handleAlignedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (AL.getNumArgs() == 0) {
    D->addAttr(::new (S.Context)
                   AlignedAttr(AL.getRange(), S.Context, true, nullptr,
                               AL.getAttributeSpellingListIndex()));
    return;
  }
  ...
  if (!AL.isPackExpansion() && S.DiagnoseUnexpandedParameterPack(E)) return;
...
void Sema::AddAlignedAttr(SourceRange AttrRange, Decl *D, Expr *E,
  ...
  if (TmpAttr.isAlignas()) {
    ...
    if (isa<ParmVarDecl>(D)) {
      ...
      if (VD->getStorageClass() == SC_Register) DiagKind = 1;
      if (VD->isExceptionVariable()) DiagKind = 2;
    } else if (const auto *FD = dyn_cast<FieldDecl>(D)) {
      if (FD->isBitField()) DiagKind = 3;
  ...
  AlignedAttr *AA = ::new (Context)
      AlignedAttr(AttrRange, Context, true, ICE.get(), SpellingListIndex);
  AA->setPackExpansion(IsPackExpansion);
  D->addAttr(AA);
}
...
void Sema::AddAlignedAttr(SourceRange AttrRange, Decl *D, TypeSourceInfo *TS,
  ...
  AlignedAttr *AA = ::new (Context)
      AlignedAttr(AttrRange, Context, false, TS, SpellingListIndex);
  AA->setPackExpansion(IsPackExpansion);
  D->addAttr(AA);
}
...
void Sema::CheckAlignasUnderalignment(Decl *D) {
  ...
  if (DiagTy->isDependentType() || DiagTy->isIncompleteType()) return;
  ...
  for (auto *I : D->specific_attrs<AlignedAttr>()) {
    if (I->isAlignmentDependent()) return;
...
bool Sema::checkMSInheritanceAttrOnDefinition(
  ...
  if (!RD->getDefinition()->isCompleteDefinition()) return false;
  ...
  if (BestCase) {
    if (RD->calculateInheritanceModel() == SemanticSpelling) return false;
  } else {
    if (RD->calculateInheritanceModel() <= SemanticSpelling) return false;
  }
...
void Sema::AddModeAttr(SourceRange AttrRange, Decl *D, IdentifierInfo *Name,
  ...
  if (Str.size() >= 4 && Str[0] == 'V') {
    ...
    if (VectorStringLength &&
      ...
      if (!InInstantiation) Diag(AttrLoc, diag::warn_vector_mode_deprecated);
    } else {
      VectorSize = 0;
    }
  }
  ...
  if (const auto *TD = dyn_cast<TypedefNameDecl>(D))
    ...
    if (OldTy.isNull()) OldTy = Context.IntTy;
  } else
  ...
  if (!OldElemTy->getAs<BuiltinType>() && !OldElemTy->isComplexType() &&
    ...
    if (!IntegralOrAnyEnumType) Diag(AttrLoc, diag::err_mode_wrong_type);
  } else if (ComplexMode) {
    if (!OldElemTy->isComplexType()) Diag(AttrLoc, diag::err_mode_wrong_type);
  } else {
    if (!OldElemTy->isFloatingType()) Diag(AttrLoc, diag::err_mode_wrong_type);
  }
...
static void handleNoDebugAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  D->addAttr(::new (S.Context) NoDebugAttr(AL.getRange(), S.Context,
                                           AL.getAttributeSpellingListIndex()));
}
...
AlwaysInlineAttr *Sema::mergeAlwaysInlineAttr(Decl *D, SourceRange Range,
  ...
  if (D->hasAttr<AlwaysInlineAttr>()) return nullptr;

  return ::new (Context)
      AlwaysInlineAttr(Range, Context, AttrSpellingListIndex);
}
...
InternalLinkageAttr *Sema::mergeInternalLinkageAttr(Decl *D,
  ...
  if (checkAttrMutualExclusion<CommonAttr>(*this, D, AL)) return nullptr;

  return ::new (Context) InternalLinkageAttr(
      AL.getRange(), Context, AL.getAttributeSpellingListIndex());
}
InternalLinkageAttr *Sema::mergeInternalLinkageAttr(
    Decl *D, const InternalLinkageAttr &AL) {
  if (const auto *VD = dyn_cast<VarDecl>(D)) {
    // Attribute applies to Var but not any subclass of it (like ParmVar,
    // ImplicitParm or VarTemplateSpecialization).
    if (VD->getKind() != Decl::Var) {
...
MinSizeAttr *Sema::mergeMinSizeAttr(Decl *D, SourceRange Range,
  ...
  if (D->hasAttr<MinSizeAttr>()) return nullptr;
...
OptimizeNoneAttr *Sema::mergeOptimizeNoneAttr(Decl *D, SourceRange Range,
  ...
  if (D->hasAttr<OptimizeNoneAttr>()) return nullptr;

  return ::new (Context)
      OptimizeNoneAttr(Range, Context, AttrSpellingListIndex);
}

static void handleAlwaysInlineAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (checkAttrMutualExclusion<NotTailCalledAttr>(S, D, AL)) return;

  if (AlwaysInlineAttr *Inline = S.mergeAlwaysInlineAttr(
          D, AL.getRange(), AL.getName(), AL.getAttributeSpellingListIndex()))
    D->addAttr(Inline);
}
...
static void handleConstantAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (checkAttrMutualExclusion<CUDASharedAttr>(S, D, AL)) return;
  const auto *VD = cast<VarDecl>(D);
...
static void handleSharedAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  if (checkAttrMutualExclusion<CUDAConstantAttr>(S, D, AL)) return;
  const auto *VD = cast<VarDecl>(D);
...
static void handleGlobalAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) CUDAGlobalAttr(
      AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}
...
static void handleCallConvAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  CallingConv CC;
  if (S.CheckCallingConvAttr(AL, CC, /*FD*/ nullptr)) return;
  ...
  switch (AL.getKind()) {
    ...
    case ParsedAttr::AT_Pascal:
      D->addAttr(::new (S.Context) PascalAttr(
          AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
      return;
    case ParsedAttr::AT_SwiftCall:
      D->addAttr(::new (S.Context) SwiftCallAttr(
          AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
      return;
    case ParsedAttr::AT_VectorCall:
      D->addAttr(::new (S.Context) VectorCallAttr(
          AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
      return;
    case ParsedAttr::AT_MSABI:
      D->addAttr(::new (S.Context) MSABIAttr(
          AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
      return;
    case ParsedAttr::AT_SysVABI:
      D->addAttr(::new (S.Context) SysVABIAttr(
          AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
      return;
    ...
    case ParsedAttr::AT_Pcs: {
      ...
      D->addAttr(::new (S.Context) PcsAttr(AL.getRange(), S.Context, PCS,
                                           AL.getAttributeSpellingListIndex()));
      return;
    }
    ...
    case ParsedAttr::AT_IntelOclBicc:
      D->addAttr(::new (S.Context) IntelOclBiccAttr(
          AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
      return;
...
static void handleSuppressAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned I = 0, E = AL.getNumArgs(); I != E; ++I) {
    ...
    if (!S.checkStringLiteralArgumentAttr(AL, I, RuleName, nullptr)) return;
...
bool Sema::CheckCallingConvAttr(const ParsedAttr &Attrs, CallingConv &CC,
  ...
  if (Attrs.isInvalid()) return true;
...
static bool isValidSwiftContextType(QualType Ty) {
  if (!Ty->hasPointerRepresentation()) return Ty->isDependentType();
...
static bool isValidSwiftErrorResultType(QualType Ty) {
  ...
  if (!Ty.getQualifiers().empty()) return false;
  ...
  switch (abi) {
    ...
    case ParameterABI::SwiftContext:
      if (!isValidSwiftContextType(type)) {
        Diag(range.getBegin(), diag::err_swift_abi_parameter_wrong_type)
            << getParameterABISpelling(abi) << /*pointer to pointer */ 0
            << type;
      }
      D->addAttr(::new (Context)
    ...
    case ParameterABI::SwiftErrorResult:
      if (!isValidSwiftErrorResultType(type)) {
        Diag(range.getBegin(), diag::err_swift_abi_parameter_wrong_type)
            << getParameterABISpelling(abi) << /*pointer to pointer */ 1
            << type;
      }
      D->addAttr(::new (Context)
...
static Expr *makeLaunchBoundsArgExpr(Sema &S, Expr *E,
                                     ...
                                     const unsigned Idx) {
  if (S.DiagnoseUnexpandedParameterPack(E)) return nullptr;

  // Accept template arguments for now as they depend on something else.
  // We'll get to check them when they eventually get instantiated.
  if (E->isValueDependent()) return E;
...
void Sema::AddLaunchBoundsAttr(SourceRange AttrRange, Decl *D, Expr *MaxThreads,
  ...
  if (MaxThreads == nullptr) return;

  if (MinBlocks) {
    MinBlocks = makeLaunchBoundsArgExpr(*this, MinBlocks, TmpAttr, 1);
    if (MinBlocks == nullptr) return;
  }
...
static void handleTypeTagForDatatypeAttr(Sema &S, Decl *D,
  ...
  D->addAttr(::new (S.Context) TypeTagForDatatypeAttr(
      AL.getRange(), S.Context, PointerKind, MatchingCTypeLoc,
      AL.getLayoutCompatible(), AL.getMustBeNull(),
      AL.getAttributeSpellingListIndex()));
}
...
static bool isValidSubjectOfOSAttribute(QualType QT) {
  if (QT->isDependentType()) return true;
  ...
  switch (K) {
    case RetainOwnershipKind::OS:
      handleSimpleAttributeOrDiagnose<OSConsumedAttr>(
          *this, VD, SR, SpellingIndex,
          isValidSubjectOfOSAttribute(VD->getType()),
    ...
    case RetainOwnershipKind::NS:
      handleSimpleAttributeOrDiagnose<NSConsumedAttr>(
          *this, VD, SR, SpellingIndex,
          isValidSubjectOfNSAttribute(VD->getType()),

          // These attributes are normally just advisory, but in ARC,
          // ns_consumed is significant.  Allow non-dependent code to contain
          // inappropriate attributes even in ARC, but require template
          // instantiations to be set up correctly.
...
bool Sema::checkNSReturnsRetainedReturnType(SourceLocation Loc, QualType QT) {
  if (isValidSubjectOfNSReturnsRetainedAttribute(QT)) return false;
...
static void handleObjCBridgeRelatedAttr(Sema &S, Decl *D,
  ...
  D->addAttr(::new (S.Context) ObjCBridgeRelatedAttr(
      AL.getRange(), S.Context, RelatedClass, ClassMethod, InstanceMethod,
      AL.getAttributeSpellingListIndex()));
}
...
static void handleObjCDesignatedInitializer(Sema &S, Decl *D,
  ...
  D->addAttr(::new (S.Context) ObjCDesignatedInitializerAttr(
      AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}

static void handleObjCRuntimeName(Sema &S, Decl *D, const ParsedAttr &AL) {
  StringRef MetaDataName;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, MetaDataName)) return;
  D->addAttr(::new (S.Context)
                 ObjCRuntimeNameAttr(AL.getRange(), S.Context, MetaDataName,
                                     AL.getAttributeSpellingListIndex()));
}
...
static void handleObjCBoxable(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (RD) {
    ObjCBoxableAttr *BoxableAttr = ::new (S.Context) ObjCBoxableAttr(
        AL.getRange(), S.Context, AL.getAttributeSpellingListIndex());
...
static void handleObjCPreciseLifetimeAttr(Sema &S, Decl *D,
  ...
  D->addAttr(::new (S.Context) ObjCPreciseLifetimeAttr(
      AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}
...
UuidAttr *Sema::mergeUuidAttr(Decl *D, SourceRange Range,
  ...
  if (const auto *UA = D->getAttr<UuidAttr>()) {
    if (UA->getGuid().equals_lower(Uuid)) return nullptr;
...
static void handleUuidAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  StringRef StrRef;
  SourceLocation LiteralLoc;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, StrRef, &LiteralLoc)) return;
...
static void handleMSInheritanceAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  MSInheritanceAttr *IA = S.mergeMSInheritanceAttr(
      D, AL.getRange(), /*BestCase=*/true, AL.getAttributeSpellingListIndex(),
...
static void handleAbiTagAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  for (unsigned I = 0, E = AL.getNumArgs(); I != E; ++I) {
    StringRef Tag;
    if (!S.checkStringLiteralArgumentAttr(AL, I, Tag)) return;
    Tags.push_back(Tag);
  }
  ...
  if (const auto *NS = dyn_cast<NamespaceDecl>(D)) {
    ...
    if (AL.getNumArgs() == 0) Tags.push_back(NS->getName());
...
static void handleMSP430InterruptAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (hasFunctionProto(D) && getFunctionOrMethodNumParams(D) != 0) {
    S.Diag(D->getLocation(), diag::warn_msp430_interrupt_attribute) << 0;
    return;
  }

  if (!getFunctionOrMethodResultType(D)->isVoidType()) {
    S.Diag(D->getLocation(), diag::warn_msp430_interrupt_attribute) << 1;
    return;
  }
  ...
  D->addAttr(::new (S.Context) MSP430InterruptAttr(
      AL.getLoc(), S.Context, Num, AL.getAttributeSpellingListIndex()));
...
static void handleMipsInterruptAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (hasFunctionProto(D) && getFunctionOrMethodNumParams(D) != 0) {
    S.Diag(D->getLocation(), diag::warn_mips_interrupt_attribute) << 0;
    return;
  }

  if (!getFunctionOrMethodResultType(D)->isVoidType()) {
    S.Diag(D->getLocation(), diag::warn_mips_interrupt_attribute) << 1;
    return;
  }

  if (checkAttrMutualExclusion<Mips16Attr>(S, D, AL)) return;
...
static void handleAVRInterruptAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkAttributeNumArgs(S, AL, 0)) return;
...
static void handleWebAssemblyImportModuleAttr(Sema &S, Decl *D,
  ...
  StringRef Str;
  SourceLocation ArgLoc;
  if (!S.checkStringLiteralArgumentAttr(AL, 0, Str, &ArgLoc)) return;
...
static void handleRISCVInterruptAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkAttributeAtMostNumArgs(S, AL, 1)) return;
...
static void handleAMDGPUFlatWorkGroupSizeAttr(Sema &S, Decl *D,
  ...
  if (!checkUInt32Argument(S, AL, MinExpr, Min)) return;

  uint32_t Max = 0;
  Expr *MaxExpr = AL.getArgAsExpr(1);
  if (!checkUInt32Argument(S, AL, MaxExpr, Max)) return;
  ...
  D->addAttr(::new (S.Context) AMDGPUFlatWorkGroupSizeAttr(
      AL.getLoc(), S.Context, Min, Max, AL.getAttributeSpellingListIndex()));
}
...
static void handleAMDGPUWavesPerEUAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) AMDGPUWavesPerEUAttr(
      AL.getLoc(), S.Context, Min, Max, AL.getAttributeSpellingListIndex()));
}
...
static void handleAMDGPUNumSGPRAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkUInt32Argument(S, AL, NumSGPRExpr, NumSGPR)) return;

  D->addAttr(::new (S.Context) AMDGPUNumSGPRAttr(
      AL.getLoc(), S.Context, NumSGPR, AL.getAttributeSpellingListIndex()));
}

static void handleAMDGPUNumVGPRAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  uint32_t NumVGPR = 0;
  Expr *NumVGPRExpr = AL.getArgAsExpr(0);
  if (!checkUInt32Argument(S, AL, NumVGPRExpr, NumVGPR)) return;

  D->addAttr(::new (S.Context) AMDGPUNumVGPRAttr(
      AL.getLoc(), S.Context, NumVGPR, AL.getAttributeSpellingListIndex()));
}
...
static void handleX86ForceAlignArgPointerAttr(Sema &S, Decl *D,
  ...
  if (VD && VD->getType()->isFunctionPointerType()) return;
...
static void handleLayoutVersion(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (!checkUInt32Argument(S, AL, AL.getArgAsExpr(0), Version)) return;
  ...
  D->addAttr(::new (S.Context) LayoutVersionAttr(
      AL.getRange(), S.Context, Version, AL.getAttributeSpellingListIndex()));
}
...
DLLImportAttr *Sema::mergeDLLImportAttr(Decl *D, SourceRange Range,
  ...
  if (D->hasAttr<DLLImportAttr>()) return nullptr;
...
DLLExportAttr *Sema::mergeDLLExportAttr(Decl *D, SourceRange Range,
  ...
  if (D->hasAttr<DLLExportAttr>()) return nullptr;
...
MSInheritanceAttr *Sema::mergeMSInheritanceAttr(
    Decl *D, SourceRange Range, bool BestCase, unsigned AttrSpellingListIndex,
    MSInheritanceAttr::Spelling SemanticSpelling) {
  if (MSInheritanceAttr *IA = D->getAttr<MSInheritanceAttr>()) {
    if (IA->getSemanticSpelling() == SemanticSpelling) return nullptr;
...
static void handleCapabilityAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) CapabilityAttr(
      AL.getRange(), S.Context, N, AL.getAttributeSpellingListIndex()));
}
...
static void handleAssertCapabilityAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(::new (S.Context) AssertCapabilityAttr(
...
static void handleAcquireCapabilityAttr(Sema &S, Decl *D,
  ...
  D->addAttr(::new (S.Context) AcquireCapabilityAttr(
      AL.getRange(), S.Context, Args.data(), Args.size(),
      AL.getAttributeSpellingListIndex()));
}
...
static void handleRequiresCapabilityAttr(Sema &S, Decl *D,
  ...
  RequiresCapabilityAttr *RCA = ::new (S.Context)
      RequiresCapabilityAttr(AL.getRange(), S.Context, Args.data(), Args.size(),
                             AL.getAttributeSpellingListIndex());
...
static bool isGlobalVar(const Decl *D) {
  if (const auto *S = dyn_cast<VarDecl>(D)) return S->hasGlobalStorage();
  return false;
}
...
static bool handleCommonAttributeFeatures(Sema &S, Decl *D,
  ...
  if (AL.getKind() == ParsedAttr::UnknownAttribute) return false;
  // Check whether the attribute requires specific language extensions to be
  // enabled.
  if (!AL.diagnoseLangOpts(S)) return true;
  // Check whether the attribute appertains to the given subject.
  if (!AL.diagnoseAppertainsTo(S, D)) return true;
  if (AL.hasCustomParsing()) return false;

  if (AL.getMinArgs() == AL.getMaxArgs()) {
    // If there are no optional arguments, then checking for the argument count
    // is trivial.
    if (!checkAttributeNumArgs(S, AL, AL.getMinArgs())) return true;
  } else {
  ...
  if (S.CheckAttrTarget(AL)) return true;
...
static void handleObjCExternallyRetainedAttr(Sema &S, Decl *D,
  ...
  if (auto *VD = dyn_cast<VarDecl>(D)) {
    ...
    if (!tryMakeVariablePseudoStrong(S, VD, /*DiagnoseFailure=*/true)) return;
...
static void ProcessDeclAttribute(Sema &S, Scope *scope, Decl *D,
  ...
  if (AL.isInvalid() || AL.getKind() == ParsedAttr::IgnoredAttribute) return;

  // Ignore C++11 attributes on declarator chunks: they appertain to the type
  // instead.
  if (AL.isCXX11Attribute() && !IncludeCXX11Attributes) return;
  ...
  if (handleCommonAttributeFeatures(S, D, AL)) return;
...
static void checkUnusedDeclAttributes(Sema &S, const ParsedAttributesView &A) {
  for (const ParsedAttr &AL : A) {
    ...
    if (AL.isUsedAsTypeAttr() || AL.isInvalid()) continue;
    if (AL.getKind() == ParsedAttr::IgnoredAttribute) continue;
...
NamedDecl *Sema::DeclClonePragmaWeak(NamedDecl *ND, IdentifierInfo *II,
  ...
  if (auto *FD = dyn_cast<FunctionDecl>(ND)) {
    ...
    if (FD->getQualifier()) NewFD->setQualifierInfo(FD->getQualifierLoc());
...
void Sema::ProcessPragmaWeak(Scope *S, Decl *D) {
  ...
  if (!WeakUndeclaredIdentifiers.empty()) {
    ...
    if (auto *VD = dyn_cast<VarDecl>(D))
      if (VD->isExternC()) ND = VD;
    if (auto *FD = dyn_cast<FunctionDecl>(D))
      if (FD->isExternC()) ND = FD;
    if (ND) {
...
static const AvailabilityAttr *getAttrForPlatform(ASTContext &Context,
  ...
  for (const auto *A : D->attrs()) {
    if (const auto *Avail = dyn_cast<AvailabilityAttr>(A)) {
      ...
      if (RealizedPlatform == TargetPlatform) return Avail;
    }
  }
  return nullptr;
}
...
static bool ShouldDiagnoseAvailabilityInContext(
    Sema &S, AvailabilityResult K, VersionTuple DeclVersion, Decl *Ctx,
  ...
  auto CheckContext = [&](const Decl *C) {
    if (K == AR_NotYetIntroduced) {
      if (const AvailabilityAttr *AA = getAttrForPlatform(S.Context, C))
        if (AA->getIntroduced() >= DeclVersion) return true;
    } else if (K == AR_Deprecated) {
      if (C->isDeprecated()) return true;
      ...
      if (const auto *MD = dyn_cast<ObjCMethodDecl>(OffendingDecl)) {
        if (const auto *Impl = dyn_cast<ObjCImplDecl>(C)) {
          if (MD->getClassInterface() == Impl->getClassInterface()) return true;
        }
      }
    }

    if (C->isUnavailable()) return true;
    return false;
  };

  do {
    if (CheckContext(Ctx)) return false;
    ...
    if (const auto *CatOrImpl = dyn_cast<ObjCImplDecl>(Ctx)) {
      if (const ObjCInterfaceDecl *Interface = CatOrImpl->getClassInterface())
        if (CheckContext(Interface)) return false;
    }
...
static bool shouldDiagnoseAvailabilityByDefault(
...
/// otherwise.
static Optional<unsigned> tryParseObjCMethodName(
    StringRef Name, SmallVectorImpl<StringRef> &SlotNames,
  ...
  for (StringRef S : SlotNames) {
    if (S.empty()) continue;
    if (!isValidIdentifier(S, AllowDollar)) return None;
  }
  return NumParams;
}
...
static Optional<AttributeInsertion> createAttributeInsertion(
    const NamedDecl *D, const SourceManager &SM, const LangOptions &LangOpts) {
  if (isa<ObjCPropertyDecl>(D))
    return AttributeInsertion::createInsertionAfter(D);
  if (const auto *MD = dyn_cast<ObjCMethodDecl>(D)) {
    if (MD->hasBody()) return None;
    return AttributeInsertion::createInsertionAfter(D);
  }
  if (const auto *TD = dyn_cast<TagDecl>(D)) {
    SourceLocation Loc =
        Lexer::getLocForEndOfToken(TD->getInnerLocStart(), 0, SM, LangOpts);
    if (Loc.isInvalid()) return None;
...
static void DoEmitAvailabilityWarning(
  ...
  switch (K) {
    case AR_NotYetIntroduced: {
      // We would like to emit the diagnostic even if -Wunguarded-availability
      // is not specified for deployment targets >= to iOS 11 or equivalent or
      ...
      if (const auto *Enclosing = findEnclosingDeclToAnnotate(Ctx)) {
        ...
        if (Enclosing->hasAttr<AvailabilityAttr>()) return;
        if (!S.getPreprocessor().isMacroDefined("API_AVAILABLE")) return;
    ...
    for (DelayedDiagnosticPool::pool_iterator i = pool->pool_begin(),
      ...
      switch (diag.Kind) {
        ...
        case DelayedDiagnostic::Access:
          ...
          if (AnyAccessFailures && isa<DecompositionDecl>(decl)) continue;
          HandleDelayedAccessCheck(diag, decl);
          if (diag.Triggered) AnyAccessFailures = true;
          break;
 ...
 public:
  ...
  static const Stmt *findLastStmtThatUsesDecl(const Decl *D,
    ...
    for (auto I = Scope->body_rbegin(), E = Scope->body_rend(); I != E; ++I) {
      const Stmt *S = *I;
      if (!Visitor.TraverseStmt(const_cast<Stmt *>(S))) return S;
    }
    return nullptr;
  }
};
...
void DiagnoseUnguardedAvailability::DiagnoseDeclAvailability(
  ...
  if (Result != AR_Available) {
    ...
    if (Result != AR_NotYetIntroduced) return;

    const AvailabilityAttr *AA =
        getAttrForPlatform(SemaRef.getASTContext(), OffendingDecl);
    VersionTuple Introduced = AA->getIntroduced();

    if (AvailabilityStack.back() >= Introduced) return;
    ...
    SourceLocation StmtEndLoc =
        SM.getExpansionRange(
            ...
            .getEnd();
    if (SM.getFileID(IfInsertionLoc) != SM.getFileID(StmtEndLoc)) return;
...
void Sema::DiagnoseAvailabilityOfDecl(NamedDecl *D,
  ...
  if (Result == AR_Available) return;

  if (Result == AR_NotYetIntroduced) {
    if (AvoidPartialAvailabilityChecks) return;
  ...
  if (const auto *MD = dyn_cast<ObjCMethodDecl>(D)) {
    if (const ObjCPropertyDecl *PD = MD->findPropertyDecl()) {
      ...
      if (PDeclResult == Result) ObjCPDecl = PD;
    }
  }
