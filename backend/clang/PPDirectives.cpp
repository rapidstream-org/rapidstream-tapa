  ...
  if (const IdentifierInfo *II = Result.getIdentifierInfo()) {
    ...
    if (SkippingUntilPragmaHdrStop && II->getPPKeywordID() == tok::pp_pragma) {
      ...
      if (II && II->getName() == "hdrstop")
        return HandlePragmaDirective(HashTok.getLocation(), PIK_HashPragma);
    }
  }
  DiscardUntilEndOfDirective();
}
...
void Preprocessor::HandleIncludeDirective(Token &HashTok,
  ...
  if (!File) {
    ...
    if (!SuppressIncludeNotFoundError) {
      ...
      if (!File) {
        auto Toks = llvm::make_unique<Token[]>(5);
        ...
        Toks[3] = FilenameTok;
        Toks[4] = NewLineTok2;
        EnterTokenStream(std::move(Toks), 5, true);
        return;
      }
    }
  }
...
void Preprocessor::HandleImportDirective(Token &HashTok,
  ...
  return HandleIncludeDirective(HashTok, ImportTok, nullptr, nullptr, true);
}
